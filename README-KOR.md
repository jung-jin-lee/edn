...
**edn** 은 확장 가능한 데이터 표기법이다.  **edn**의 확대집합(superset)은 클로저에서 프로그램을 표현하기 위해
사용되어지고, Datomic과 다른 어플리케이션의 데이터 전송 형식으로 사용되어진다. 이 명세는 다른 언어의 독자와 
작가의 구현을 용이하게 돕기 위함과 기타 용도로 다른 특정한 사용 사례와 별도로
**edn**을 묘사합니다.

**edn**은 풍부한 내장 요소들과 다른 측면의 확장 요소들의 정의를 지원합니다. 그런 편의가 없는 데이터 형식의 사용자들은 기본 집합에 포함되지 않은 요소들을 전달하기 위해 관례나 문맥에 의존해야 합니다.
이것은 어플리케이션 로직을 상당히 복잡하게 만들고, 형식의 명백한 단순성을 저버립니다. **edn**은 단순하지만, 관례나 복잡한 문맥-의존(context-sensitive) 로직 없이 어플리케이션들의 요구사항들을 충족시키지에 충분히 강력합니다.

**edn**은 _values_ 의 수송을 위한 시스템입니다. 그것은 타입 시스템이 아니고, 스키마를 가지고 있지 않습니다. 그것은 객체(object)를 표현하기 위한 시스템이 아니고 - 참조 타입이 없고, **edn**의 본문 어딘가에 두개의 동일한 요소를 가질 것으로 소비자는 예상하지 않습니다.
만일 리더(reader) 구현이 그러한 약속을 한 방법으로 빠지지 않는 한, 읽힐 때 유일한 객체(object)의 고유값(identity)을 넘겨줄 것입니다. 
그러므로 결과값은 불변으로 고려되어야 하고, 리더(reader) 구현은 가능한 범위까지 이를 보증한 값을 넘겨주어야 합니다.
